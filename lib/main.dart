...
class ApiClient {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    final data = await post('/login', body: {'email': email, 'password': password});
...
class _AuthGateState extends State<AuthGate> {
  ...
  Widget build(BuildContext context) {
    ...
    final u = user;
    if (u == null) {
      return RoleSelectScreen(api: client, onLogin: _onLogin);
    }
    final role = u['role'] as String? ?? '';
    if (role == 'admin') return AdminShell(api: client, user: u, onLogout: _logout);
    if (role == 'accountant') return AccountantShell(api: client, user: u, onLogout: _logout);
    return CustomerShell(api: client, user: u, onLogout: _logout);
  }
}

// ---------------------------------------------------------------------------
// Role select — choose staff or customer
// ---------------------------------------------------------------------------

class RoleSelectScreen extends StatelessWidget {
  const RoleSelectScreen({required this.api, required this.onLogin, super.key});
  final ApiClient api;
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                children: [
                  const BankHeader(subtitle: 'Smart Queue System'),
                  ...
                  _RoleCard(
                    ...
                    title: 'Customer',
                    subtitle: 'Submit transactions & track queue',
                    color: AppColors.gold,
                    onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => CustomerLoginScreen(api: api, onLogin: onLogin),
                      ),
                    ),
                  ),
                  ...
                  _RoleCard(
                    ...
                    title: 'Staff / Manager',
                    subtitle: 'Manage queue, windows & reports',
                    color: AppColors.blue,
                    onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => StaffLoginScreen(api: api, onLogin: onLogin),
                      ),
                    ),
                  ),
...
class CustomerLoginScreen extends StatefulWidget {
  const CustomerLoginScreen({required this.api, required this.onLogin, super.key});
  final ApiClient api;
  final ValueChanged<Map<String, dynamic>> onLogin;
...
class _CustomerLoginScreenState extends State<CustomerLoginScreen> {
  ...
  Future<void> _login() async {
    setState(() { _loading = true; _error = null; _success = null; _verificationRequired = false; });
    try {
      final user = await widget.api.login(_email.text.trim(), _password.text);
      if (user['role'] != 'customer') {
        throw const ApiException('This login is for customers only. Use the Staff Login button.');
      }
      widget.onLogin(user);
    } on ApiException catch (e) {
      if (e.statusCode == 403) {
        setState(() { _verificationRequired = true; _error = e.message; });
      } else {
        setState(() => _error = e.message);
      }
    } catch (_) {
  ...
  Future<void> _googleLogin() async {
    ...
    try {
      ...
      if (idToken == null) throw const ApiException('Google sign-in failed: no ID token.');
  ...
  Future<void> _resend() async {
    ...
    try {
      ...
      setState(() { _success = 'Verification email sent. Check your inbox.'; _verificationRequired = false; });
    } on ApiException catch (e) {
      setState(() => _error = e.message);
    } finally {
      if (mounted) setState(() => _loading = false);
...
class CustomerSignupScreen extends StatefulWidget {
  const CustomerSignupScreen({required this.api, required this.onLogin, super.key});
  final ApiClient api;
  final ValueChanged<Map<String, dynamic>> onLogin;
        ...
        child: Center(
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              constraints: const BoxConstraints(maxWidth: 460),
              child: AppCard(
                child: _done ? _VerificationSentView(email: _email.text, api: widget.api, onLogin: widget.onLogin) : _RegisterForm(
                  name: _name,
                  email: _email,
                  password: _password,
                  loading: _loading,
                  ...
                  onLoginTap: () => Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                      builder: (_) => CustomerLoginScreen(api: widget.api, onLogin: widget.onLogin),
                    ),
                  ),
                ),
              ),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 14),
        AppTextField(controller: password, label: 'Password (min 6 chars)', obscureText: true),
...
class _VerificationSentViewState extends State<_VerificationSentView> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const Icon(Icons.mark_email_read_outlined, size: 64, color: AppColors.gold),
        const SizedBox(height: 16),
        ...
        const SizedBox(height: 8),
        const Text('Click the link in the email, then come back and sign in.',
            textAlign: TextAlign.center,
...
class CustomerShell extends StatefulWidget {
  const CustomerShell({required this.api, required this.user, required this.onLogout, super.key});
  final ApiClient api;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          NavigationDestination(icon: Icon(Icons.list_alt_outlined), selectedIcon: Icon(Icons.list_alt), label: 'My Queue'),
          NavigationDestination(icon: Icon(Icons.add_circle_outline), selectedIcon: Icon(Icons.add_circle), label: 'New Request'),
...
class _CustomerTransactionsPageState extends State<CustomerTransactionsPage> {
  ...
  void initState() {
    super.initState();
    _fetch();
    _timer = Timer.periodic(const Duration(seconds: 5), (_) => _fetch(silent: true));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_txs.isEmpty) {
      return const Center(child: Text('No transactions yet.\nUse "New Request" to get started.', textAlign: TextAlign.center));
    }
    final calledTx = _txs.firstWhere(
        (tx) => tx['status'] == 'pending', orElse: () => null);
    return Column(
      children: [
        if (calledTx != null)
          Container(
                        ...
                        children: [
                          ...
                          if (tx['queue_number'] != null)
                            Chip(
                              ...
                              labelStyle: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.gold),
...
class _CustomerNewRequestPageState extends State<CustomerNewRequestPage> {
  ...
  Future<void> _submit() async {
    if (_windowId == null) { setState(() => _error = 'Please select a window.'); return; }
    if (_photo == null) { setState(() => _error = 'Please upload your photo.'); return; }
    if (_sigController.isEmpty) { setState(() => _error = 'Please draw your signature.'); return; }

    setState(() { _loading = true; _error = null; _success = null; });
    try {
      final sigBytes = await _sigController.toPngBytes();
      if (sigBytes == null) throw const ApiException('Could not export signature.');
      final tmpSig = File('${Directory.systemTemp.path}/sig_${DateTime.now().millisecondsSinceEpoch}.png');
      ...
      setState(() => _success = 'Transaction submitted! You are now in the queue.');
        ...
        children: [
          ...
          AppCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Step 1 — Choose service',
                ...
                const SizedBox(height: 16),
                const Text('Select window',
                    style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black54, fontSize: 12)),
                const SizedBox(height: 8),
                if (_windows.isEmpty)
                  const Text('Loading windows...', style: TextStyle(color: Colors.black54))
                else
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: _windows.map((w) {
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: sel ? AppColors.gold.withOpacity(0.15) : Colors.white,
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(color: sel ? AppColors.gold : Colors.grey.shade300),
                          ),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('${w['name']}', style: TextStyle(fontWeight: FontWeight.w700, color: sel ? AppColors.gold : Colors.black87)),
                              Text('${w['waiting_count'] ?? 0} waiting', style: const TextStyle(fontSize: 11, color: Colors.black54)),
                            ],
                          ),
                        ),
                      );
          ...
          AppCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Step 2 — Fill details',
                    style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black54, fontSize: 12)),
                const SizedBox(height: 14),
                AppTextField(controller: _accountNumber, label: 'Account Number', keyboardType: TextInputType.number),
                const SizedBox(height: 12),
                AppTextField(controller: _accountHolder, label: 'Account Holder Name'),
                const SizedBox(height: 12),
                AppTextField(controller: _amount, label: 'Amount (ETB)', keyboardType: TextInputType.number),
                const SizedBox(height: 12),
                AppTextField(controller: _amountWords, label: 'Amount in Words', hint: 'e.g. Five Hundred Birr'),
                const SizedBox(height: 12),
                AppTextField(controller: _depositedBy, label: 'Deposited by (optional)'),
                if (_type == 'transfer') ...[
                  const SizedBox(height: 12),
                  AppTextField(controller: _toAccount, label: 'Recipient Account Number'),
                ],
              ],
            ),
          ),
          ...
          AppCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Step 3 — Upload photo',
                    style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black54, fontSize: 12)),
                const SizedBox(height: 12),
                if (_photo != null)
                  Stack(
                    ...
                    child: Container(
                      ...
                      child: const Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.photo_camera_outlined, color: AppColors.gold, size: 32),
                            SizedBox(height: 6),
                            Text('Tap to upload photo', style: TextStyle(color: AppColors.gold, fontWeight: FontWeight.w600)),
                          ],
                        ),
                      ),
                    ),
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    const Expanded(
                      child: Text('Step 4 — Draw signature',
                          style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black54, fontSize: 12)),
                    ),
                    TextButton(
                ...
                const SizedBox(height: 6),
                const Text('Sign in the box above',
          ...
          if (_success != null) ...[SuccessBox(_success!), const SizedBox(height: 10)],
...
class _CustomerReceiptsPageState extends State<CustomerReceiptsPage> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      ...
      if (mounted) setState(() { _receipts = data; _loading = false; });
    } catch (_) {
      if (mounted) setState(() => _loading = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_receipts.isEmpty) {
      return const Center(
        child: Text('No receipts yet.\nReceipts appear here after transactions are completed.', textAlign: TextAlign.center),
      );
    }
        ...
        itemBuilder: (_, i) {
          ...
          return AppCard(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${r['transaction']?['type'] ?? 'Transaction'} Receipt'.toUpperCase(),
                    style: const TextStyle(fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text('Amount: ${money(r['transaction']?['amount'])}',
                    style: const TextStyle(color: Colors.black54)),
                if (r['receipt_url'] != null) ...[
                  const SizedBox(height: 10),
                  ClipRRect(
                    borderRadius: BorderRadius.circular(8),
                    child: Image.network('${r['receipt_url']}', fit: BoxFit.cover),
                  ),
                ],
              ],
            ),
...
class AdminShell extends StatefulWidget {
  const AdminShell({required this.api, required this.user, required this.onLogout, super.key});
  final ApiClient api;
  final Map<String, dynamic> user;
  final VoidCallback onLogout;
...
class _AdminShellState extends State<AdminShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Manager: ${widget.user['name'] ?? ''}'),
...
class _AdminOverviewState extends State<AdminOverview> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final report = await widget.api.get('/transactions/daily?type=all') as Map;
      setState(() {
        liveQueue = queue;
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          SectionTitle(title: 'Live Queue', trailing: loading ? 'Loading...' : '${liveQueue.length} windows'),
              ...
              child: Column(
                ...
                children: [
                  Text('${window['name'] ?? 'Window'}',
                      style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                  Text('Staff: ${window['accountant']?['name'] ?? 'Unassigned'}'),
                  const Divider(),
                  ...((window['transactions'] as List? ?? []).map((tx) => TransactionListTile(tx: tx))),
...
class _StaffManagementState extends State<StaffManagement> {
  ...
  Future<void> addStaff() async {
    ...
    try {
      await widget.api.post('/accountants', body: {
        'name': name.text.trim(),
        'email': email.text.trim(),
        'password': password.text,
        'window': window.text.trim(),
      });
      name.clear(); email.clear(); password.clear(); window.clear();
      await refresh();
          ...
          AppCard(
            child: Column(
              children: [
                ...
                const SizedBox(height: 12),
                AppTextField(controller: window, label: 'Window name', hint: 'Example: Window 1'),
                if (message != null) ...[const SizedBox(height: 12), InfoBox(message!)],
                const SizedBox(height: 16),
                FilledButton.icon(
          ...
          for (final item in windows)
            AppCard(
              ...
              child: ListTile(
                ...
                title: Text('${item['name'] ?? ''}'),
                subtitle: Text('Staff: ${item['accountant']?['name'] ?? 'Unassigned'}'),
              ),
            ),
        ],
      ),
...
class _AdminTransactionsState extends State<AdminTransactions> {
  ...
  Future<void> refresh() async {
    setState(() => loading = true);
    final report = await widget.api.get('/transactions/$period?type=$type') as Map;
    setState(() {
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        Row(children: [
          Expanded(child: AppDropdown(value: period, values: const ['daily', 'weekly', 'monthly', 'yearly'], onChanged: (v) => setState(() => period = v))),
          const SizedBox(width: 10),
          Expanded(child: AppDropdown(value: type, values: const ['all', 'deposit', 'withdraw', 'transfer'], onChanged: (v) => setState(() => type = v))),
        ]),
        const SizedBox(height: 12),
        FilledButton(onPressed: loading ? null : refresh, child: Text(loading ? 'Loading...' : 'Load Report')),
        const SizedBox(height: 16),
        ...
        if (transactions.isEmpty) const AppCard(child: Text('No transactions found.')),
      ],
    );
  }
}
...
class _AdminSettingsState extends State<AdminSettings> {
  ...
  Future<void> save() async {
    try {
      await widget.api.post('/admin/settings', body: {
        'withdraw_min': min.text,
        'withdraw_max': max.text,
      });
    ...
    return ListView(
      ...
      children: [
        ...
        AppCard(
          child: Column(
            children: [
              AppTextField(controller: min, label: 'Minimum withdraw amount', keyboardType: TextInputType.number),
              const SizedBox(height: 12),
              AppTextField(controller: max, label: 'Maximum withdraw amount', keyboardType: TextInputType.number),
              if (message != null) ...[const SizedBox(height: 12), InfoBox(message!)],
              const SizedBox(height: 16),
              FilledButton.icon(onPressed: save, icon: const Icon(Icons.save_outlined), label: const Text('Save Settings')),
            ],
          ),
        ),
      ],
...
class AccountantShell extends StatefulWidget {
  const AccountantShell({required this.api, required this.user, required this.onLogout, super.key});
  final ApiClient api;
  final Map<String, dynamic> user;
  final VoidCallback onLogout;
...
class _AccountantShellState extends State<AccountantShell> {
  ...
  void initState() {
    super.initState();
    refresh();
    timer = Timer.periodic(const Duration(seconds: 5), (_) => refresh(silent: true));
  }
  ...
  Future<void> refresh({bool silent = false}) async {
    ...
    try {
      ...
      setState(() {
        queue = data;
        if (selected != null) {
          selected = data.cast<dynamic>().where((tx) => tx['id'] == selected!['id']).firstOrNull ?? selected;
        }
      });
    } finally {
  ...
  Future<void> sendReceipt() async {
    ...
    await widget.api.uploadReceipt('/receipts/${tx['id']}', 'image', File(picked.path));
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            SectionTitle(title: 'Queue', trailing: loading ? 'Loading...' : '${queue.length} active'),
            ...
            if (queue.isEmpty) const AppCard(child: Text('No customers in your queue.')),
            if (selected != null) ...[
              const SizedBox(height: 12),
...
class AppDropdown extends StatelessWidget {
  const AppDropdown({required this.value, required this.values, required this.onChanged, super.key});
  final String value;
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text('${tx['user']?['name'] ?? 'Customer'}',
          ...
          DetailRow(label: 'Account number', value: '${tx['account_number'] ?? ''}'),
          DetailRow(label: 'Account holder', value: '${tx['account_holder'] ?? ''}'),
          DetailRow(label: 'Amount words', value: '${tx['amount_words'] ?? ''}'),
          DetailRow(label: 'Deposited by', value: '${tx['deposited_by'] ?? ''}'),
          DetailRow(label: 'Recipient account', value: '${tx['to_account'] ?? ''}'),
          DetailRow(label: 'Queue number', value: '${tx['queue_number'] ?? ''}'),
        ],
      ),
    );
  }
