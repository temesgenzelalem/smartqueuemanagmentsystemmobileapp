...
export default function CustomerDashboard() {
  ...
  const handleSignature = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    setSignature(file); setSignaturePreview(URL.createObjectURL(file));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!photo) { showToast("Please upload your photo", "error"); return; }
    if (!signature) { showToast("Please upload your signature", "error"); return; }
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              ...
              <div 
                ...
                className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center cursor-pointer transition-all hover:border-opacity-80"
                style={{ borderColor: colors.accent, backgroundColor: signaturePreview ? "#FFFBEB" : colors.bgSecondary }}>
                {signaturePreview ? (
                  <div className="relative">
                    <img src={signaturePreview} alt="signature preview" className="h-20 object-contain rounded-xl shadow" />
                    <button type="button" onClick={(e) => { e.stopPropagation(); setSignature(null); setSignaturePreview(null); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm">✕</button>
                  </div>
                ) : (
                  <>
                    <span className="text-4xl mb-3">✍️</span>
                    <span className="text-sm font-semibold" style={{ color: colors.accent }}>Click to upload signature</span>
                    <span className="text-xs mt-1" style={{ color: colors.textSecondary }}>JPG, PNG up to 2MB</span>
                  </>
                )}
              </div>
              <input ref={signatureRef} type="file" accept="image/*" className="hidden" onChange={handleSignature} />
            </div>
